...
Measures *PowerSampler wrapper overhead* rather than raw DummySource cost:

A) direct monotonic loop + DummySource.read_once()
B) PowerSampler(DummySource)

PowerSampler self-overhead = B_CPU - A_CPU
...
from power_monitor import DummySource, PowerSampler  # noqa: E402
...
def _run_sampler(duration_sec: int, interval_ms: int) -> float:
    sampler = PowerSampler(source=DummySource(), interval_ms=interval_ms, timeout_ms=TIMEOUT_MS)
    start_cpu = time.process_time()
    sampler.start()
    try:
